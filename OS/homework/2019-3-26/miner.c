...
void* mine(void* arg){
    if(current_miner_count > INITIAL_MINER_COUNT) {
    ...
    while(!are_minerals_empty(minerals_status) && !done){
        for(int mineral_index = 0; mineral_index < mineral_blocks_count; mineral_index++){
            ...
            if(pthread_mutex_trylock(&(mineral_blocks[mineral_index].mutex)) == 0){
...
//                            map_minerals,  minerals, current_miner_count, warriors);
