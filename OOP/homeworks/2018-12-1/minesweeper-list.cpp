...
public:
    ...
    //trying list implementation
    point* at(int row, int col){
        if(col < 0 ||  col >= width || row < 0 || row >= height){
            return NULL;
        }
        //scary stuff incomming:
        for(list<list<point>>::iterator it = matrix.begin(); it != matrix.end(); it++) {
            for (list<point>::iterator it2 = (*it).begin(); it2 != (*it).end(); it2++) {
                point* element = &(*it2);
                if (element->get_x() == col && element->get_y() == row) {
                    return element;
                }
            }
        }
